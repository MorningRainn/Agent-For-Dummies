# 大模型与 AI 基础：把高频术语讲成人话

## 常用 AI 概念

### Transformer 与注意力

文本会先被切分为 Token，再映射为向量（embedding）；位置编码补上顺序信息。Transformer 通过注意力机制让每个 Token 能从其他 Token 中选择与当前任务最相关的信息，因此可以并行处理序列。

- **Q（Query）**：当前 Token 想查找什么信息。
- **K（Key）**：每个 Token 可供匹配的索引特征。
- **V（Value）**：匹配后应取回的内容。
- **缩放点积注意力**：先计算 `QK^T` 的相关性，除以 `sqrt(d_k)` 以稳定数值，再经 Softmax 得到权重，最后对 V 加权求和。

![QKV illustration](../assets/pasted-image-20260116092844.png)

![Scaled dot-product attention](../assets/pasted-image-20260116094321.png)

多头注意力会并行使用多组 Q、K、V 投影，使不同的 head 能关注不同的语法或语义关系。解码器在生成时使用因果掩码，避免看到尚未生成的后续 Token；交叉注意力则使解码器可以读取编码器输出。

![Attention scores](../assets/pasted-image-20260116093731.png)

![Attention output](../assets/pasted-image-20260116094040.png)
